...
class V100 extends RestController{ 
    ...
    *****/
    ...
    public function search_get() {        
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            $page               =   $this->input->get('page');
            $response['movie']              =   $this->api_v100_model->get_movie_search_result($q,$page);
            $response['tvseries']           =   $this->api_v100_model->get_tvseries_search_result($q,$page);
            $response['tv_channels']        =   $this->api_v100_model->get_tv_channel_search_result($q,$page);
        else:
            $response['status']     = 'error';
   ...
   // get check_validated_subscription_plan
    ...
    public function store_payment_info_post() {
        ...
        $data['transaction_id']             =   $this->input->post('payment_info');
    ...
    public function firebase_auth_post() {        
        ...
        if($uid !='' && $uid !=NULL):         
            $login_status               = $this->api_v100_model->validate_user_by_uid($uid);        
            if ($login_status):
